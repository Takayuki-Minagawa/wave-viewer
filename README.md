...
1列の数値データ（加速度データ）を読み込み、時系列波形・フーリエスペクトル・応答スペクトルをリアルタイムで表示・分析するツールです。
...
- **変位波形**（速度の積分により自動計算）
- X軸: 時間 [sec]
- Y軸: 加速度 [cm/s2, m/s², g] / 速度 [m/s または cm/s] / 変位 [cm]
...
- 速度データのCSVエクスポート
- 変位データのCSVエクスポート
- 応答スペクトル（Sa/Sv/Sd, h=2/3/5%）のCSVエクスポート
- 全データ（時間・加速度・速度・変位）の一括エクスポート
- 加速度単位が `cm/s2` の場合、速度は `cm/s` で出力
- Excel対応（BOM付きUTF-8）
...
- **上段**: 時系列波形グラフ
- **中段**: フーリエスペクトルグラフ
- **下段**: 応答スペクトル（Sa/Sv/Sd, h=2/3/5%）
- **統計情報**: 各種統計値とピーク周波数
- **エクスポート**: 波形CSV + 応答スペクトルCSV
- **コンソール**: K-netデータの場合、観測点情報などが表示されます

## 📘 簡易マニュアル

1. `ファイル選択` またはドラッグ&ドロップでデータを読み込みます。  
2. 読み込み後に自動解析され、加速度・速度・変位・フーリエ・応答スペクトルが表示されます。  
3. `cm/s2` データでは速度表示/出力は自動で `cm/s` になります。  
4. グラフはホイールでズーム、ドラッグでパン、`リセット` ボタンで初期表示に戻せます。  
5. `データエクスポート` から `応答スペクトル` を選ぶと、`Sa/Sv/Sd`（h=2/3/5%）をCSV出力できます。  
...
- ✅ サンプリング周波数の自動設定
- ✅ スケールファクターの自動適用
- ✅ データ単位をcm/s2に自動設定（K-net生データの `gal` を換算）
- ✅ ヘッダー17行の自動スキップ
- ✅ 観測点情報の抽出（コンソールに出力）
...
| データ単位 | 加速度の単位 | **cm/s2** | **cm/s2（自動）** |

> **Note**: K-net/KiK-net形式のファイルでは、設定パラメータは自動的に適用されます。
>  
> **Note**: 加速度単位が `cm/s2` のとき、速度は `cm/s` で表示・エクスポートされます（`gal` と同等）。
...
- ✅ **フーリエスペクトルの対数スケールを横軸（周波数軸）に変更**
  - 「対数スケール」チェックボックスの動作をY軸の対数変換からX軸（周波数軸）の対数表示に変更
  - チェックボックスのラベルを「横軸を対数表示」に変更（英語: "Log Frequency Axis"）
  - 対数軸時のティックラベルを代表的な周波数値に制限して見やすく表示
  - チェックボックス切替時に軸タイプとティック表示ルールが正しく同期するよう修正
...
- ✅ **加速度単位のデフォルトを変更**
  - デフォルト単位を `m/s²` から `cm/s2` に変更
  - 単位コンボボックスとグラフ表示の初期単位を `cm/s2` に統一
- ✅ **README / 簡易マニュアル更新**
  - 単位説明を `cm/s2` 基準に更新
...
- ✅ **応答スペクトル解析を追加**
  - 加速度応答スペクトル（Sa）/速度応答スペクトル（Sv）/変位応答スペクトル（Sd）を追加
  - 周期 `0.02s〜10.0s`、等比200分割（201点）、`h=2/3/5%` で算定
- ✅ **速度単位の自動切替を追加**
  - 加速度単位が `cm/s2`（`gal`相当）の場合、速度表示・CSV出力を `cm/s` に自動切替
- ✅ **応答スペクトルCSVエクスポートを追加**
  - `Sa/Sv/Sd` を `h=2/3/5%` で一括出力
- ✅ **README / 簡易マニュアル更新**
  - 現在の機能仕様に合わせて操作説明を更新

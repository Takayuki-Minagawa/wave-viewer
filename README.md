...
- **変位波形**（速度の積分により自動計算）
- X軸: 時間 [sec]
- Y軸: 加速度 [m/s², gal, g] / 速度 [m/s または cm/s] / 変位 [cm]
...
3. `gal` データでは速度表示/出力は自動で `cm/s` になります。  
...
>  
> **Note**: 加速度単位が `gal` のとき、速度は `cm/s` で表示・エクスポートされます。
...
- ✅ **速度単位の自動切替を追加**
  - 加速度単位が `gal` の場合、速度表示・CSV出力を `cm/s` に自動切替
- ✅ **応答スペクトルCSVエクスポートを追加**
